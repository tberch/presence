...
class AudioEventDetector {
    constructor(options = {}) {
        ...
        this.options = {
            ...
            debugVisualizer: false,       // Whether to show debug visualizer (default: false)
            onEvent: null,                // Callback for detected events
            onError: (error) => console.error(error), // Error callback
            statusElement: null,          // Element to display status messages
            ...options
        };
        ...
        // Debug visualization (only initialized if debugVisualizer is true)
    ...
    updateStatus(message) {
        ...
        console.log('[AudioDetector]', message);
    }
    ...
    checkAudioSupport() {
        ...
        // Older browsers might have it with prefixes
        else if (navigator.getUserMedia || navigator.webkitGetUserMedia || 
                navigator.mozGetUserMedia || navigator.msGetUserMedia) {
            
            // Create a shim for older browser versions
            navigator.mediaDevices = {};
            navigator.mediaDevices.getUserMedia = function(constraints) {
                // Get appropriate getUserMedia based on browser
                const getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia ||
                                    navigator.mozGetUserMedia || navigator.msGetUserMedia;
                                    
                // Return a Promise for compatibility with newer API
    ...
    async initialize() {
        try {
            this.updateStatus('Initializing audio detector...');
            
            // Check for browser compatibility
            const mediaDevices = this.checkAudioSupport();
            if (!mediaDevices) {
            ...
            if (!AudioContext) {
                throw new Error("Your browser doesn't support AudioContext API");
            }
            ...
            // Initialize debug visualizer only if explicitly enabled
            ...
            this.updateStatus('Audio detector initialized successfully');
            return true;
        } catch (error) {
            const errorMessage = 'AudioEventDetector initialization failed: ' + error.message;
    ...
    start() {
        if (!this.isInitialized) {
            this.updateStatus('Error: AudioEventDetector not initialized');
            return false;
        }
        
        this.isRecording = true;
        this.updateStatus('Listening for audio events...');
        this.processAudio();
        return true;
    }
    ...
    stop() {
        ...
        this.updateStatus('Audio detection stopped');
    }
    ...
    processAudio() {
        ...
        // Draw debug visualization only if enabled and context exists
        if (this.debugContext && this.options.debugVisualizer) {
     ...
     * (This method is retained but will only be called if debugVisualizer is true)
     */
    drawDebugVisualizer() {
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { AudioEventDetector, createDoorbellDetector, createKnockingDetector };
} else {
  // Add to window for browser usage
  window.AudioEventDetector = AudioEventDetector;
  window.createDoorbellDetector = createDoorbellDetector;
  window.createKnockingDetector = createKnockingDetector;
}
